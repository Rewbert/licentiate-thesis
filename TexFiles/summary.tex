...
The results of this thesis indicate that there is absolutely space for a high-level language with domain-specific abstractions
...
%     \item What is the difference between a general purpose, and a domain-specific, programming language?
% \end{LaTeXdescription}

% To begin to address this question we state what a general-purpose language is, "A computer language that is broadly
% applicable across application domains, and lacks specialized features for a particular domain".

% Does this definition imply that it is completely arbitrary, which language we should choose for a specific project? We should be
...
% language and a domain-specific language. Haskell, as another example, is a general-purpose language whose support for custom algebraic data types and pattern-matching
% makes it ideal for a program that manipulates trees, such as compilers and interpreters.
...
% can be expressed in e.g. C or Haskell, but since these languages lacks abstractions for expressing this, a lot more code has
% to be written to achieve the same behavior. More code means more bugs, higher maintenance costs, and perhaps a larger software team.
% We conclude this paragraph by emphasizing that using domain-specific abstractions in high-level languages, help developers to focus
% on the intended behavior of an application, rather than details of the implementation.

% Some application domains are harder to provide a high-level language for than others. For example, if applications in this domain
% need to execute on more niche hardware, the choice of hardware can influence what can execute or not. If the target
% platform is a small microcontroller, memory constraints will influence what languages could realisticly execute on it, as different
% languages have different memory footprints. Another reason could be that the language has a managed runtime, and that this runtime
% is written in such a way that it makes assumptions of the platform it runs on. In this case, properties of the programming language,
% their memory requirements or their implementation, is what stops us.

% Another example of potentially problematic hardware, is hardware enforced trusted-execution environments (TEE). While TEEs are
% supported on more conventional machines, where memory or implementation is less likely to be an issue, there are other limiting
% factors. The toolchains that enables development of TEEs expects the developer to write low-level C code, which is
% error-prone and has no domain-specific abstractions. Furthermore, a unique programing model where an application is made up of
% two components further hinders application development, as this is not a model easilly expressed in existing high-level
% languages. In this case, properties of the platform hinders us.

% In this thesis, I explore how we can overcome obstacles imposed by either the choice programming language itself, or imposed by
% the target platform. I do this by leveraging an existing high-level general-pupose language, Haskell, to implement
% domain-specific languages. The following two sections will illustrate how the lack of domain-specific abstractions can
% lead to an unecessarily complicated implementation, and how my research has made these domains more accessible. We then cover some
% important background topics, after which the two papers that make up this thesis follow.

\section{Domain One: Programs with Temporal Behaviors}
\todo{Title of section is weird}
...
whether a pixel (denoted by a \code{(Int,Int)} tuple) lies inside or outside of the shape.
...
The meaning of each function is encoded in the \code{run} function, where syntax is turned into semantics.
...
A shallow embedding will produce a result in a semantic domain, whereas a deep embedding does not necessarily have to
do that. Once the syntax is built, the syntax can be converted to a result in a semantic domain with a \code{run} function
